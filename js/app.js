...
import { groupKnockoutMatches, normalizeCupFormat, calculateGroupStandings, formatVictoryTime, knockoutSeedPreview, phaseInfo } from './cup.js?v=4';
...
function renderGroups(){
  ...
  $('#groupRule').innerHTML=`<span>${format.qualifiers_per_group} passam</span><span>+${format.points_win} vitória</span><span>${format.points_draw} empate</span><span>0×3 ${format.points_loss_three_crowns}</span><span>PTS › SG › tempo</span>`;
  ...
  grid.innerHTML=groups.map(label=>{
    ...
    const mobile=rows.length?rows.map((r,i)=>`<div class="group-mobile-row ${hasResult&&i<format.qualifiers_per_group?'qualifying':''}"><span class="group-rank">${i+1}</span><div class="group-mobile-name"><b>${escapeHtml(r.name)}</b><small>J ${r.j} · SG ${r.sg>0?'+':''}${r.sg}${r.time_complete?` · ${formatVictoryTime(r.win_time_seconds)}`:''}</small></div><strong>${r.pts}<small>PTS</small></strong></div>`).join(''):`<div class="group-mobile-empty">Nenhum participante</div>`;
    return `<article class="group-card"><div class="group-card-head"><div><span>GRUPO</span><h3>${escapeHtml(label)}</h3></div><small>${format.qualifiers_per_group} vagas</small></div><div class="group-table-wrap"><table aria-label="Grupo ${escapeHtml(label)}"><thead><tr><th>#</th><th>Participante</th><th>J</th><th>SG</th><th>Tempo</th><th>PTS</th></tr></thead><tbody>${body}</tbody></table></div><div class="group-mobile-list" aria-label="Classificação do Grupo ${escapeHtml(label)}">${mobile}</div></article>`;
  }).join('');
}
...
function fmtDate(value){
  ...
  try{
    ...
    const hasDefinedTime=date.getHours()!==0||date.getMinutes()!==0;
    if(!hasDefinedTime)return day;
...
function dateOnlyLabel(value){
  ...
  if(diff===2)return `Depois de amanhã · ${formatted}`;
  return formatted;
}

function roundDateLabel(matches){
  const dated=matches.filter(m=>m.scheduled_at);
  const unique=[...new Map(dated.map(m=>[dateKey(m.scheduled_at),m.scheduled_at])).values()];
  if(!unique.length)return 'Data a definir';
  if(unique.length===1)return dateOnlyLabel(unique[0]);
  return `${unique.length} datas`;
}

function groupRoundNumber(label){
  const text=String(label||'');
  const match=text.match(/(?:rodada|fase)\s*(\d+)/i)||text.match(/\b(\d+)\b/);
  return match?Number(match[1]):Number.MAX_SAFE_INTEGER;
}

function compareMatches(a,b){
  const ad=a.scheduled_at?new Date(a.scheduled_at).getTime():Number.MAX_SAFE_INTEGER;
  const bd=b.scheduled_at?new Date(b.scheduled_at).getTime():Number.MAX_SAFE_INTEGER;
  if(ad!==bd)return ad-bd;
  const an=participantName(a.participant_a),bn=participantName(b.participant_a);
  return an.localeCompare(bn,'pt-BR',{numeric:true,sensitivity:'base'}) || String(a.id).localeCompare(String(b.id));
}
...
function renderMatchCard(m,{compactStage=false}={}){
  ...
  const isGroup=(m.stage_type||'KNOCKOUT')==='GROUP';
  const stage=compactStage&&isGroup?'CONFRONTO':isGroup?`Grupo ${m.group_label||'—'} · ${m.round_label}`:m.round_label;
...
function renderGroupMatchRounds(matches){
  const rounds=new Map();
  for(const match of matches){
    const label=String(match.round_label||'Rodada a definir').trim()||'Rodada a definir';
    if(!rounds.has(label))rounds.set(label,[]);
    rounds.get(label).push(match);
  }
  return [...rounds.entries()]
    .sort(([a],[b])=>groupRoundNumber(a)-groupRoundNumber(b)||a.localeCompare(b,'pt-BR',{numeric:true,sensitivity:'base'}))
    .map(([roundLabel,roundMatches])=>{
      const groups=new Map();
      for(const match of roundMatches){
        const label=String(match.group_label||'—');
        if(!groups.has(label))groups.set(label,[]);
        groups.get(label).push(match);
      }
      const groupHtml=[...groups.entries()]
        .sort(([a],[b])=>a.localeCompare(b,'pt-BR',{numeric:true,sensitivity:'base'}))
        .map(([groupLabel,groupMatches])=>`<section class="match-group-section" aria-label="Grupo ${escapeHtml(groupLabel)}"><div class="match-group-head"><h4>Grupo ${escapeHtml(groupLabel)}</h4><span>${groupMatches.length} ${groupMatches.length===1?'partida':'partidas'}</span></div><div class="match-group-grid">${groupMatches.sort(compareMatches).map(m=>renderMatchCard(m,{compactStage:true})).join('')}</div></section>`)
        .join('');
      return `<section class="match-round-section" aria-label="${escapeHtml(roundLabel)}"><header class="match-round-head"><div><span>FASE DE GRUPOS</span><h3>${escapeHtml(roundLabel)}</h3></div><strong>${escapeHtml(roundDateLabel(roundMatches))}</strong></header><div class="match-round-groups">${groupHtml}</div></section>`;
    }).join('');
}

function renderKnockoutMatchRounds(matches){
  const phases=groupKnockoutMatches(matches);
  return phases.map(phase=>`<section class="match-round-section knockout-round" aria-label="${escapeHtml(phase.label)}"><header class="match-round-head"><div><span>MATA-MATA</span><h3>${escapeHtml(phase.label)}</h3></div><strong>${escapeHtml(roundDateLabel(phase.matches))}</strong></header><div class="match-group-grid knockout-match-grid">${phase.matches.sort(compareMatches).map(m=>renderMatchCard(m)).join('')}</div></section>`).join('');
}

function renderMatches(){
  const list=$('#matchesList');
  let matches=[...cupMatches()];
  ...
  const groupMatches=matches.filter(m=>(m.stage_type||'KNOCKOUT')==='GROUP');
  const knockoutMatches=matches.filter(m=>(m.stage_type||'KNOCKOUT')==='KNOCKOUT');
  list.innerHTML=`${renderGroupMatchRounds(groupMatches)}${renderKnockoutMatchRounds(knockoutMatches)}`;
}
