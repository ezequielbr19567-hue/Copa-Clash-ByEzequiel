...
function standings(){
  const league = data.competitions.find(c=>c.kind==='LEAGUE' && c.active) || data.competitions.find(c=>c.kind==='LEAGUE');
  if(!league) return [];
  const eligible = data.teams.filter(t=>t.team_type===league.mode);
  const rows = new Map(eligible.map(t=>[t.id,{id:t.id,name:t.name,j:0,v:0,e:0,d:0,cf:0,cs:0,sg:0,tresZero:0,pts:0}]));
  data.matches.filter(m=>m.competition_id===league.id && m.status==='FINISHED' && m.crowns_a!=null && m.crowns_b!=null).forEach(m=>{
...
function renderStandings(){
  ...
  body.innerHTML=rows.map((r,i)=>`<tr><td><span class="rank ${i<3?'top':''}">${i+1}</span></td><td><b>${escapeHtml(r.name)}</b></td><td>${r.j}</td><td>${r.v}</td><td>${r.e}</td><td>${r.d}</td><td>${r.cf}</td><td>${r.cs}</td><td>${r.sg>0?'+':''}${r.sg}</td><td>${r.tresZero}</td><td><b class="pts">${r.pts}</b></td></tr>`).join('');
}

function renderBracket(){
  const cup=data.competitions.find(c=>c.kind==='CUP'&&c.active)||data.competitions.find(c=>c.kind==='CUP');
  const league=data.competitions.find(c=>c.kind==='LEAGUE'&&c.active)||data.competitions.find(c=>c.kind==='LEAGUE');
  const leagueMatches=data.matches.filter(m=>league && m.competition_id===league.id);
  ...
  const phases=groupCupMatches(data.matches.filter(m=>cup && m.competition_id===cup.id));
  ...
  $('#cupStage').textContent=champion?'Temos um campeão':phases.length?'A disputa está montada':'Rumo ao mata-mata';
  $('#cupContext').textContent=champion?champion.name+' conquistou o título. Reviva os confrontos desta campanha.':pending?pending+' '+(pending===1?'partida restante':'partidas restantes')+' na classificação. A disputa pelo título ganha forma a cada resultado.':phases.length?'Os confrontos estão aqui. Explore cada fase e acompanhe quem segue na disputa.':'A próxima etapa merece sua torcida. Os confrontos aparecerão aqui quando forem cadastrados.';
  ...
  if(!phases.length){
    ...
    nav.innerHTML='';
    el.innerHTML='<div class="cup-waiting"><span class="waiting-symbol" aria-hidden="true">✦</span><div><h4>O próximo capítulo está chegando.</h4><p>'+escapeHtml(appearance.empty_cup)+'. Assim que os confrontos forem definidos, acompanhe cada duelo por aqui.</p></div><span class="waiting-badge">Aguardando confrontos</span></div>';
    return;
  }
  ...
  function draw(){
    ...
    }).join('')+'</div>'+(phase.key==='final'?'<div class="final-destination"><span aria-hidden="true">✦</span><strong>'+escapeHtml(champion?champion.name:'O título espera seu dono.')+'</strong><span>'+(champion?'Campeão da Copa':'A última batalha. A maior conquista.')+'</span></div>':'')+'</section>').join('');
    el.scrollLeft=0;
...
function renderMatches(){
  ...
  if(currentFilter==='LEAGUE'||currentFilter==='CUP') matches=matches.filter(m=>comp(m.competition_id)?.kind===currentFilter);
...
async function loadData(){
  ...
  try{
    ...
    const failed=queries.slice(0,6).find(q=>q.error); if(failed) throw failed.error;
    if (queries[6].error) console.warn(queries[6].error);
    appearance = normalizeSettings(queries[6].data?.settings);
    applySettings(appearance);
    loaded = true;
    data={players:queries[0].data||[],teams:queries[1].data||[],team_members:queries[2].data||[],competitions:queries[3].data||[],matches:queries[4].data||[],games:queries[5].data||[]};
  }catch(err){
