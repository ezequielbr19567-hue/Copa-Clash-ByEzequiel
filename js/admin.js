...
import {
  ...
  calculateGroupStandings, knockoutSeedPlan, winnerOf, roundLabelForSize,
  roundCodeForSize, roundRobinSchedule, parseVictoryTime, formatVictoryTime
} from './cup.js?v=3';
import { defaults, normalizeSettings, applySettings, imageUrl } from './settings.js?v=3';
...
async function refresh(msg){
  try { await loadData(); renderDashboard(); if(msg) notice('success',msg); }
...
async function syncAutomaticKnockout(cupId) {
  ...
  const initialStarted=initialExisting.some(m=>m.status==='FINISHED');
...
function groupsHtml(){
  ...
  const plan=knockoutSeedPlan({entries,matches:cupMatches(cup.id),format:f,nameFor});
  ...
  return `<section class="admin-card"><h2>Distribuição e jogos dos grupos</h2><p>Para sua regra atual, use 5 pessoas por grupo. Os 4 melhores avançam.</p><label class="field">Copa<select id="groupCupSelect">${cupOptions(cup.id)}</select></label><div class="group-admin-toolbar"><span>${f.groups.length} grupos · ${f.qualifiers_per_group} classificados por grupo</span><div class="inline-actions"><button id="autoGroups" type="button" class="button secondary small">Distribuir automaticamente</button><button id="generateGroupMatches" type="button" class="button secondary small">Gerar jogos dos grupos</button></div></div><form id="groupsForm"><div class="data-list">${data.teams.length?data.teams.map(t=>`<div class="data-row group-assignment"><div><b>${escapeHtml(t.name)}</b><small>Participante</small></div><select name="participant-${t.id}" data-group-assignment="${t.id}"><option value="">Fora desta Copa</option>${f.groups.map(g=>`<option value="${escapeHtml(g)}" ${entryMap.get(t.id)===g?'selected':''}>Grupo ${escapeHtml(g)}</option>`).join('')}</select></div>`).join(''):'<div class="empty">Cadastre participantes primeiro.</div>'}</div><div class="form-actions"><button class="button primary" ${data.teams.length?'':'disabled'}>Salvar distribuição</button></div></form></section><section class="admin-card"><h2>Mata-mata automático</h2><div class="notice ${plan.ready?'success':'info'}">${escapeHtml(plan.ready?'Todos os grupos estão completos. A chave pode ser sincronizada automaticamente.':plan.reason)}</div><div class="crossing-rule"><b>Cruzamento:</b> 1º do Grupo ${escapeHtml(f.groups.at(-1)||'4')} × 4º do Grupo ${escapeHtml(f.groups[0]||'1')}; 2º × 3º; e assim por diante. O outro lado cruza Grupo ${escapeHtml(f.groups.at(-2)||'3')} × Grupo ${escapeHtml(f.groups[1]||'2')}.</div><button id="syncKnockout" class="button primary full" type="button" ${plan.ready?'':'disabled'}>Sincronizar mata-mata agora</button><div class="mini-standings-grid">${standingsHtml}</div></section>`;
}
