...
export function normalizeCupFormat(value = {}) {
  ...
  return {
    ...
    points_draw: safeInt(raw.points_draw, defaultCupFormat.points_draw, -9, 20),
...
export function knockoutSeedPlan({ entries = [], matches = [], format, nameFor = id => String(id) }) {
  const f = normalizeCupFormat(format);
  const groups = f.groups;
  const total = groups.length * f.qualifiers_per_group;
  if (groups.length < 2 || groups.length % 2 !== 0) return { ready:false, reason:'A geração automática precisa de uma quantidade par de grupos.', pairs:[] };
  if (total < 2 || (total & (total - 1)) !== 0) return { ready:false, reason:'O total de classificados precisa formar uma chave de 2, 4, 8, 16 ou 32 participantes.', pairs:[] };

  const standings = new Map();
  for (const group of groups) {
    const rows = calculateGroupStandings({ entries, matches, groupLabel:group, format:f, nameFor });
    if (rows.length < f.qualifiers_per_group) return { ready:false, reason:`O Grupo ${group} ainda não tem participantes suficientes.`, pairs:[], standings };
    if (!groupIsComplete({ entries, matches, groupLabel:group })) return { ready:false, reason:`O Grupo ${group} ainda tem partidas pendentes.`, pairs:[], standings };
  ...
  return { ready:true, reason:'', pairs, standings, size:total, round_label:roundLabelForSize(total), round_code:roundCodeForSize(total) };
}
