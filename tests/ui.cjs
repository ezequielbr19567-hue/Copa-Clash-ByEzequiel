...
assert(/syncAutomaticKnockout/.test(admin),'admin must advance knockout automatically');
assert(/video_url/.test(admin)&&/renderVideo/.test(app),'match videos must be editable and rendered');
...
assert(html.includes('js/config.js?v=3'),'config script should be cache-busted');
